...
if ($outcome->rowCount() > 0) 
{
    ...
    header("Location: ../view/customerRequest.php?result=success&id={$line['requestID']}&status={$line['status']}&nome={$line['clientName']}&email={$line['email']}&telefone={$line['telefone']}&cpf={$line['cpf']}&dispositivo={$line['dispositive']}&descricao={$line['problemDescription']}");
    exit();
} 
else 
{
    header("Location: ../view/customerRequest.php?result=not_found");
    exit();
}
?>